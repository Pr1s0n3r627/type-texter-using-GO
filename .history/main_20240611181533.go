...
func main() {
	...
	textEdit.Resize(fyne.NewSize(500, 300)) // Set the desired size of the textEdit widget

	// Arrange the widgets using a VBox
	content := container.NewBorder(
		container.NewVBox(textEdit) // Top: nil to ensure textEdit is always at the top
		nil, // Right
		nil, // Bottom
		nil, // Left
		container.NewVBox(typeButton), // Center: VBox to arrange textEdit and typeButton vertically with a separator
	)

	w.SetContent(content)
	w.Resize(fyne.NewSize(500, 500)) // Adjust window size accordingly
